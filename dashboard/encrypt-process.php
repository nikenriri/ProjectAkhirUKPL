...
if (isset($_POST['encrypt_now'])) {
    ...
    $query1  = mysqli_query($connect, $sql1) or die(mysqli_error($link));

    $sql2   = "select * from file where file_url =''";
    $query2  = mysqli_query($connect, $sql2) or die(mysqli_error($link));

    $url   = $finalfile . ".rda";
    $file_url = "file_encrypt/$url";

    $sql3   = "UPDATE file SET file_url ='$file_url' WHERE file_url=''";
    $query3  = mysqli_query($connect, $sql3) or die(mysqli_error($link));
